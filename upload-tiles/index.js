...
const downloadAll = async () => {
  const { coordsLevel, minZoomLevel, startZoomLevel, coordList, startCoords, resources } = params;

  let totaltiles = 0;
  let count = 0;
  let failed = 0;
  let i = 0;
  let len = coordList.length;

  if (!coordList) {
    coordList = [];
  }
  if (!coordList.length && startCoords) {
    coordList.push(startCoords);
  }

  for (let startCoords of coordList) {
    i++;
    if (!startCoords) {
      error("!!!ERROR!!!");
      ...
      for (const x of XCoords) {
        for (const y of YCoords) {
          ...
          try {
            ...
            count++;
            log(`${i}/${len} Done: (${((count/totaltiles)*100).toFixed(2)}%) ${count}/${totaltiles}; failed: ${failed}`)
          }
